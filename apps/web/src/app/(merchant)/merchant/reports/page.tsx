...
export default function ReportsPage() {
  ...
  const handleExportTransactions = async () => {
    try {
      ...
      await exportTransactions(selectedPeriod, user?.merchantId);
    } catch (error: any) {
  ...
  const handleExportPayouts = async () => {
    try {
      ...
      await exportPayouts(selectedPeriod, user?.merchantId);
    } catch (error: any) {
      // Don't show alert if it's an auth error (redirect will happen)
      if (!error.message?.includes('session has expired') && !error.message?.includes('log in again')) {
  ...
  const handleExportReport = () => {
    ...
    if (reportType === 'transactions') {
      ...
      // For financial reports, export both transactions and payouts
      handleExportPayouts();
    } else {
      handleExportTransactions();
    }
